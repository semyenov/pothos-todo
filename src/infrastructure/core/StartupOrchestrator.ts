...
} from "../orchestration/ServiceDependencyGraph.js";
import {
  TypedEventEmitter,
...
type StartupOrchestratorEventMap = ServiceEventMap & {
  "startup:started": [{
    timestamp: Date;
    stages: number;
    services: number;
...
export class StartupOrchestrator extends TypedEventEmitter<StartupOrchestratorEventMap> {
  private readonly registry: EnhancedServiceRegistry;
  private readonly dependencyGraph: ServiceDependencyGraph;
    ...
    try {
      ...
      for (const stage of stages) {
        ...
        try {
          ...
          if (
            abortOnFailure ||
            (!stage.skipOnTimeout && error instanceof Error)
          ) {
            throw error;
          } else {
            logger.warn(`⚠️ Stage failed but continuing: ${stage.name}`, {
              error,
            });
          }
        }
    ...
    for (let i = 0; i < serviceNames.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map((serviceName) =>
          this.registry.restartService(serviceName, "rolling-restart")
        )
      );
  ...
  optimizeStartup(profiles: StartupProfile[]): {
    ...
    if (chronicBottlenecks.length > 0) {
      recommendations.push(
        `Chronic bottlenecks detected: ${chronicBottlenecks.join(
          ", "
        )}. Consider increasing timeouts or optimizing these services.`
      );
    }

    // Analyze parallel efficiency
    const avgParallelEfficiency =
      profiles.reduce((sum, p) => sum + p.parallelEfficiency, 0) /
      profiles.length;

    if (avgParallelEfficiency < 60) {
      recommendations.push(
        `Low parallel efficiency (${avgParallelEfficiency.toFixed(
          1
        )}%). Consider restructuring dependency graph to enable more parallelism.`
      );
    }
    ...
    const stageStats = new Map<
      string,
      { totalTime: number; count: number; failures: number }
    >();
    ...
    const avgCurrentDuration =
      profiles.reduce((sum, p) => sum + p.totalDuration, 0) / profiles.length;
    const criticalPath = this.dependencyGraph.calculateCriticalPath();
    const expectedImprovement = Math.max(
      0,
      ((avgCurrentDuration - criticalPath.totalTime) / avgCurrentDuration) * 100
    );

    recommendations.push(
      `Estimated startup time improvement: ${expectedImprovement.toFixed(
        1
      )}% (from ${avgCurrentDuration}ms to ~${criticalPath.totalTime}ms)`
    );
  ...
  private async executeStage(
    ...
    const stageServices = stage.services.filter((name) =>
    ...
    for (const group of groups) {
      if (parallel && group.length > 1) {
        ...
        await Promise.all(
          group.map((serviceName) =>
            this.startServiceWithRetry(serviceName, stage)
          )
        );
      } else {
  ...
  private async startServiceWithRetry(
    ...
    for (let attempt = 1; attempt <= retryPolicy.attempts; attempt++) {
      try {
        ...
        if (attempt > 1) {
          ...
          logger.info(
            `Retrying ${serviceName} (attempt ${attempt}/${retryPolicy.attempts}) after ${delay}ms`
          );
          await new Promise((resolve) => setTimeout(resolve, delay));
        }
        ...
        logger.info(
          `✅ Service started: ${serviceName} (${duration}ms, attempt ${attempt})`
        );
        return;
      } catch (error) {
        lastError = error as Error;
        logger.warn(
          `Failed to start ${serviceName} (attempt ${attempt}/${retryPolicy.attempts})`,
          { error }
        );
      }
    }
    ...
    throw new Error(
      `Failed to start ${serviceName} after ${retryPolicy.attempts} attempts: ${lastError?.message}`
    );
  }
  ...
  private async checkHealthGate(stage: StartupStage): Promise<void> {
    ...
    logger.info(`🔍 Checking health gate for stage: ${stage.name}`, {
      required,
      optional,
    });
    ...
    while (Date.now() - startTime < timeout) {
      ...
      const requiredHealthy = required.every((service) =>
        healthy.includes(service)
      );
      ...
      if (requiredHealthy) {
        ...
        logger.info(
          `✅ Health gate passed for stage: ${stage.name} (${duration}ms)`
        );
        return;
      }
    ...
    const unhealthyRequired = required.filter((service) =>
      unhealthy.includes(service)
    );

    if (unhealthyRequired.length > 0) {
      throw new Error(
        `Health gate failed for stage ${
          stage.name
        }: unhealthy required services: ${unhealthyRequired.join(", ")}`
      );
    }
  }
  ...
  private getServiceGroups(services: string[]): string[][] {
    ...
    while (remaining.size > 0) {
      ...
      for (const service of remaining) {
        ...
        const hasUnstartedDeps = deps.required.some((dep) =>
          remaining.has(dep)
        );
      ...
      if (group.length === 0) {
        const service = Array.from(remaining)[0];
        if (service) {
  ...
  private async generateProfile(
    ...
    const serviceTimings = new Map<
      string,
      { duration: number; stage: string }
    >();

    for (const stage of this.profile.stages || []) {
      for (const service of stage.services) {
        if (service.duration > 10000) {
          // Services taking more than 10s
          bottlenecks.push({
    ...
    if (parallelEfficiency < 50) {
      recommendations.push(
        "Consider restructuring dependencies to enable more parallel execution"
      );
    }

    if (bottlenecks.length > 0) {
      recommendations.push(
        `Optimize slow services: ${bottlenecks
          .map((b) => b.service)
          .join(", ")}`
      );
    }
    ...
    if (totalDuration > 120000) {
      // More than 2 minutes
      recommendations.push(
        "Overall startup time is high. Consider lazy loading non-critical services"
      );
    }
